...
/*
val List = List("Alice", "John", "Dina", "Valentin") println(List(1))
println(List.head)
println(List.tail)
for (name <- List) {println(name)}
*/
// 2-a) Then write another function to compute the factorial via reading from list. For instance, you will get list as (1,2,3,4,5) then multiply them together and compute the factorial.
// 2-b) Use the reduce method (h​ ttps://www.geeksforgeeks.org/scala-reduce-function/​) and re-compute the factorial number.
// 2-c) Extend the previous code to generate a list from a number (6 turns into list(1,2,3,4,5,6)) then compute the factorial.
